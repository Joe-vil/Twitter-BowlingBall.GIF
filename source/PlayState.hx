...
class PlayState extends MusicBeatState
{
	...
	public static var pinDifficulty:Int = 1;
	...
	var darkHue:FlxSprite;
	var pinrating:FlxSprite;
	var strikeBG:FlxSprite;

	var imageRating:String = 'rating';
	var ratingX:Int = 20;
	var deathRatingLimit:Int = 9;
	...
	override public function create()
	{
		...
		{
			case 'alley':
			if(SONG.song.toLowerCase() == "high-strike")
				{
					strikeBG = new FlxSprite(-636, -1279);
					strikeBG.frames = Paths.getSparrowAtlas('alley/bg');
					strikeBG.antialiasing = ClientPrefs.globalAntialiasing;
					strikeBG.animation.addByPrefix('1', 'BG0000', 24, true);
					strikeBG.animation.addByPrefix('2', 'BG0007', 24, true);
					strikeBG.updateHitbox();
					strikeBG.alpha = 0;
					add(strikeBG);
					}
				alley = new BGSprite('alley/floor', -204, 244, 1, 1);
				add(alley);
		}
		...
		if(curStage == 'alley')
			{
				if(pinDifficulty == 1)
					{imageRating = 'rating';}
				if(pinDifficulty == 2)
					{imageRating = 'rating-hard'; ratingX = 30; deathRatingLimit = 6;}
				if(pinDifficulty == 3)
					{imageRating = 'rating-turkey'; ratingX = 45; deathRatingLimit = 3;}
				
				pinrating = new FlxSprite(ratingX, 35);
				pinrating.frames = Paths.getSparrowAtlas('alley/' + imageRating);
				pinrating.antialiasing = ClientPrefs.globalAntialiasing;
				if(pinDifficulty == 1 || pinDifficulty == 2 || pinDifficulty == 3)
				{
				pinrating.animation.addByPrefix('0', 'idle', 24, true);
				pinrating.animation.addByPrefix('1', 'one', 24, true);
				pinrating.animation.addByPrefix('2', 'two', 24, true);
				if(pinDifficulty == 1 || pinDifficulty == 2 )
				pinrating.animation.addByPrefix('3', 'three', 24, true);
				pinrating.animation.addByPrefix('4', 'four', 24, true);
				pinrating.animation.addByPrefix('5', 'five', 24, true);
				pinrating.animation.addByPrefix('6', 'six', 24, true);
				if(pinDifficulty == 1)
				pinrating.animation.addByPrefix('7', 'seven', 24, true);
				pinrating.animation.addByPrefix('8', 'eight', 24, true);
				pinrating.animation.addByPrefix('9', 'nine', 24, true);
				}
				pinrating.animation.play('0');
				pinrating.cameras = [camRating];
				pinrating.updateHitbox();
				
				darkHue = new FlxSprite().loadGraphic(Paths.image('alley/hue'));
				darkHue.updateHitbox();
				darkHue.x = -14;
				darkHue.y = -139;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (player < 1 && middleScroll) targetAlpha = 0;
	...
	var pinRatingHold:Bool = true;
	var gameCamZoom:Float = 0.035;
	var hudCamZoom:Float = 0.05;
	var zoomBeat:Float = 4;
	var camAngle:Bool = false;

	function ratingPinCounter()
		{
			if(!pinRatingHold)
				{
				var missesNum:String = Std.string(songMisses);
				if(songMisses > 0 && songMisses < deathRatingLimit)
					{
						pinrating.animation.play(missesNum);
					}
				if(songMisses > deathRatingLimit)
					{
						health = 0;
					}
				}
	...
	function camMoveWithNotes(X:Int, Y:Int)
		{
			var val1:Float = X;
			var val2:Float = Y;
	...
	public var areYouThePin:Bool = true;
	var xx:Int = 960;
	var yy:Int = 325;
	var ofs:Int = 20;
	var zoomRate:Float = 0.5;
	var strikeMulti:Float = 0;
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'balls intro':
				...
				FlxTween.tween(darkHue, {alpha: 0}, 0.5, {ease: FlxEase.quadInOut});
				FlxTween.tween(camRating, {alpha: 1}, 0.5, {ease: FlxEase.quadInOut});
				dad.playAnim('balls intro', true);
			...
			case 'Bowling Character':

				var lastAlpha:Float = boyfriend.alpha;
				var alphalast:Float = dad.alpha;

				if(areYouThePin)
					{
						areYouThePin = false;
						if(!boyfriendMap.exists('pin')) 
							{
								addCharacterToList('pin', 1);
								addCharacterToList('balls', 0);
							}
							trace('switch to balls');
			
							boyfriend.alpha = 0.00001;
							boyfriend = boyfriendMap.get('balls');
							dad.alpha = 0.00001;
							dad = dadMap.get('pin');
							...
							xx = 555;
							yy = 370;
							defaultCamZoom = 1.3 + strikeMulti;
							zoomRate = 0.5;
					}
				else
					{
						areYouThePin = true;
						if(!dadMap.exists('balls')) 
							{
								addCharacterToList('pin', 0);
								addCharacterToList('balls', 1);
							}
							trace('switch back to pin');
			
							boyfriend.alpha = 0.00001;
							boyfriend = boyfriendMap.get('pin');
							dad.alpha = 0.00001;
							dad = dadMap.get('balls');
							...
							xx = 950;
							yy = 325;
							defaultCamZoom = 0.9 + strikeMulti;
							zoomRate = 2.5;
					}

				FlxTween.tween(FlxG.camera, {zoom: defaultCamZoom}, zoomRate, {ease: FlxEase.quadInOut});
				boyfriend.alpha = 1;
				dad.alpha = 1;

			case 'strike bg':
				FlxTween.tween(strikeBG, {alpha: 1}, 0.2, {ease: FlxEase.quadInOut});

			case 'ofs sing':
				ofs = Std.parseInt(value1);
	...
	override function beatHit()
	{
		...
		if (camZooming && FlxG.camera.zoom < 1.55 && ClientPrefs.camZooms && curBeat % zoomBeat == 0)
		{
			FlxG.camera.zoom += gameCamZoom;
			camHUD.zoom += hudCamZoom;
		}
		...
		switch (curStage)
		{
			case 'alley':
				if(SONG.song.toLowerCase() == "high-strike")
					{
							//why did i hard code all this? i have no idea lmao
							if (curBeat % 0.5 == 0)
								{
									strikeBG.animation.play('1', true);
									if(camAngle)
										camHUD.angle = -2;
								}
							if (curBeat % 2 == 0)
								{
									strikeBG.animation.play('2', true);
									if(camAngle)
										camHUD.angle = 2;
								}
							if (curBeat == 36)
							...
							if (curBeat == 132 || curBeat == 137 || curBeat == 140)
								{
									camHUD.angle = 5;
								}
							if (curBeat == 133 || curBeat == 136 || curBeat == 141)
								{
									camHUD.angle = -5;
								}
							if (curBeat == 134 || curBeat == 138 || curBeat == 142)
								{
									camHUD.angle = 0;
								}
							if (curBeat == 143)
